...
import { aaveloop, asset, deployer, expectInPosition, expectOutOfPosition, fundOwner, getPrice, initFixture, owner, reward } from "./test-base";
...
describe("AaveLoop E2E Tests", () => {
  ...
  const LTV = 8250;
  const MIN_SAFE_GAS_HEALTH_FACTOR = bn18(LTV / 8000);

  beforeEach(async () => {
    await resetNetworkFork();
    await initFixture();
    expect(await aaveloop.methods.getLTV().call()).bignumber.eq(bn(LTV), `assuming ${LTV} LTV`);
  ...
  it("Last day to exit safely under current market conditions", async () => {
    const ITERATIONS = 15;
    const EXPECTED_DAYS_TO_SAFE_GAS = 365;

    // iterations will result in free liquidity of 5% (+-1%) of PRINCIPAL
    const enterLiquidity = bn(await aaveloop.methods.enterPositionFully(ITERATIONS).call({ from: owner }));
    expect(enterLiquidity).bignumber.closeTo(await asset.amount(PRINCIPAL * 0.05), await asset.amount(PRINCIPAL * 0.01));
    await aaveloop.methods.enterPositionFully(ITERATIONS).send({ from: owner });

    await mineBlock(60 * 60 * 24 * EXPECTED_DAYS_TO_SAFE_GAS);

    expect((await aaveloop.methods.getPositionData().call()).healthFactor).bignumber.closeTo(MIN_SAFE_GAS_HEALTH_FACTOR, bn18("0.001"));
  ...
  [
    { iterations: 0, expectedLeverage: 1 },
    { iterations: 1, expectedLeverage: 1.8 },
    { iterations: 10, expectedLeverage: 4.5 },
    { iterations: 15, expectedLeverage: 4.7 },
    { iterations: 20, expectedLeverage: 4.8 },
  ].map(({ iterations, expectedLeverage }) =>
    it(`Fully enter & exit with ${iterations} iterations, x${expectedLeverage} expected leverage`, async () => {
      await aaveloop.methods.enterPositionFully(iterations).send({ from: owner });
      await expectInPosition(PRINCIPAL, expectedLeverage);
  ...
  it("Show me the money", async () => {
    await aaveloop.methods.enterPosition(await asset.amount(PRINCIPAL), 15).send({ from: owner });
    await expectInPosition(PRINCIPAL, 4);

    await mineBlocks(60 * 60 * 24, 15); // on eth
    ...
    const rewardBalance = await reward.methods.balanceOf(owner).call();
    expect(rewardBalance).bignumber.gt(zero).closeTo(pending, bn(pending).muln(0.01));
  ...
  it("partial exit, lower leverage", async () => {
    await aaveloop.methods.enterPositionFully(15).send({ from: owner });
  ...
  it("Liquidity decrease over time", async () => {
    await aaveloop.methods.enterPositionFully(10).send({ from: owner });
  ...
  it("When low liquidity, must provide additional collateral and exit in multiple txs", async () => {
    ...
    const redeemable = bn(await aaveloop.methods.getLiquidity().call())
      .muln(1e4)
      .divn(LTV);
    await aaveloop.methods._redeemSupply(redeemable.subn(10)).send({ from: owner }); // redeem 99.99999%
    ...
    expect((await aaveloop.methods.getPositionData().call()).healthFactor).bignumber.closeTo(ether, bn18(0.05));
