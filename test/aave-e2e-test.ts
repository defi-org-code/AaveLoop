...
describe("AaveLoop E2E Tests", () => {
  ...
  beforeEach(async () => {
    ...
    await initFixture();
    // expect(await aaveloop.methods.getLTV().call()).bignumber.eq(bn(ASSUME_LTV), `assuming ${ASSUME_LTV / 10000} LTV`);
  ...
  it.only(`Full E2E ${DAYS_TO_SAFE_GAS} days exit safely under current market conditions`, async () => {
    await aaveloop.methods.enterAll().send({ from: owner });
    ...
    console.log("✨", fmt6(await aaveloop.methods.getLiquidity().call()), fmt6(await aaveloop.methods.getSupplyBalance().call()));

    await mineBlock(60 * 60 * 24 * DAYS_TO_SAFE_GAS);

    console.log("💰", fmt6(await aaveloop.methods.getLiquidity().call()), fmt6(await aaveloop.methods.getSupplyBalance().call()));

    await aaveloop.methods.exit().send({ from: owner });
